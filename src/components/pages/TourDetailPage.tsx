...
export default function TourDetailPage({ tour, similar }: TourDetailPageProps) {
  ...
  const factRow = "flex items-start gap-3 border-b border-cream/10 py-3.5 last:border-b-0";
  const factIcon = "lic mt-0.5 flex-none text-[18px] text-gold";
  const factLabel = "text-[11px] uppercase tracking-[0.12em] text-cream/50";
  const factValue = "mt-0.5 text-[15px] font-semibold text-cream";
    ...
    <main>
      <div className="bg-gradient-to-b from-[#001833] to-navy pb-10 pt-32">
        <div className={ui.wrap}>
          <Link
            href="/tours"
            className="inline-flex items-center gap-2 text-[13px] font-semibold text-cream/60 transition-colors hover:text-gold"
          >
            <span className="lic">
              <ArrowLeft />
            </span>
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            ...
            <span className="flex items-center gap-1.5 text-[13px] font-semibold text-cream/70">
              <span className="lic">
                <Clock />
              </span>
              {tour.duration[lang]}
        ...
        <div className={cx(ui.wrap, "grid grid-cols-1 gap-12 lg:grid-cols-[1.6fr_1fr] lg:items-start")}>
          ...
          <div>
            ...
            <Reveal className="mt-12">
              ...
              <p className="mt-6 text-[17px] leading-[1.7] text-cream/82">
                ...
                <ol className="mt-8 border-l border-gold/25">
                  {d.program.map((step, i) => (
                    <li key={i} className="relative pb-8 pl-8 last:pb-0">
                      <span className="absolute -left-[7px] top-1 h-3.5 w-3.5 rounded-full border-2 border-gold bg-navy" />
                      ...
                      <p className="mt-2 text-[15.5px] leading-relaxed text-cream/80">{step.text[lang]}</p>
                    </li>
                  ))}
                </ol>
              </Reveal>
                  ...
                  <Reveal>
                    ...
                    <ul className="mt-5 space-y-3">
                      {d.included.map((item, i) => (
                        <li key={i} className="flex items-start gap-3 text-[15px] leading-snug text-cream/80">
                  ...
                  <Reveal delay={1}>
                    ...
                    <ul className="mt-5 space-y-3">
                      {d.notIncluded.map((item, i) => (
                        <li key={i} className="flex items-start gap-3 text-[15px] leading-snug text-cream/60">
                          <span className="lic mt-0.5 flex-none text-[18px] text-cream/40">
                            <X />
                          </span>
                          {item[lang]}
                        </li>
            ...
            {d?.tips && d.tips.length > 0 && (
              <Reveal className="mt-14 rounded border border-gold/25 bg-gold/5 p-7">
                ...
                <ul className="mt-5 space-y-3">
                  {d.tips.map((tip, i) => (
                    <li key={i} className="flex items-start gap-3 text-[15px] leading-relaxed text-cream/80">
          ...
          <Reveal className="lg:sticky lg:top-28" delay={1}>
            <div className="rounded border border-cream/12 bg-navy-2 p-7">
              <div className="text-[13px] uppercase tracking-[0.12em] text-cream/50">
              ...
              {d?.priceTable && d.priceTable.length > 0 && (
                <table className="mt-6 w-full border-collapse text-[14px]">
                  <thead>
                    <tr className="border-b border-cream/12 text-left text-[11px] uppercase tracking-[0.1em] text-cream/50">
                  ...
                  <tbody>
                    {d.priceTable.map((row, i) => (
                      <tr key={i} className="border-b border-cream/8 last:border-b-0">
                        <td className="py-2.5 text-cream/80">{row.group[lang]}</td>
                        <td className="py-2.5 text-right font-semibold text-cream">{row.price}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <p className="mt-5 text-[13px] leading-relaxed text-cream/55">{t.tourPage.priceNote}</p>
            ...
            <div className="mt-6 overflow-hidden rounded border border-cream/12">
              <KazMap />
            </div>
          </Reveal>
        </div>
      ...
      {/* similar tours */}
      {similar.length > 0 && (
        <section className={cx(ui.sec, "bg-cream-2 pt-0 text-ink")}>
          <div className={ui.wrap}>
